...
function highlightJSON(json: string): string {
  ...
  return escaped.replace(
    ...
    (match) => {
      if (/^"/.test(match)) {
        if (/:$/.test(match)) return `<span style="color:#e06c75">${match}</span>`;
...
export function AltViewClient({ id, version, initialData }: Props) {
  const router = useRouter();

  const [dataName, setDataName] = useState(initialData?.data_name ?? "");
  const [entities, setEntities] = useState<Record<string, unknown>[]>(initialData?.entities ?? []);
    ...
    async function load() {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/dataset/alternate/result?dataset_id=${id}&version_id=${version}&page=${page}&limit=25`
        );
        if (!res.ok) return;
        ...
        setDataName(data.data_name ?? `Dataset ${id}`);
      } catch (e) {
      ...
      <div>
        ...
        {!loading && (
          <p className="text-sm text-muted-foreground mt-1">
            {total.toLocaleString()} entities · {dataName || `Dataset ${id}`}
          </p>
        )}
      </div>
