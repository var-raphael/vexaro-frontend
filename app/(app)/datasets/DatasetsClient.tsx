...
function getApiURL(dataset: Dataset): string {
  ...
  return `${API_BASE}/${dataset.dataset_id}/${slug}/active/${alt}`;
}
...
function CloneModal({ dataset, open, onClose }: CloneModalProps) {
  ...
  function validateForm(): boolean {
    ...
    if (len === 0) { setExtractError("Extract description is required"); return false; }
    if (len < EXTRACT_MIN) { setExtractError(`Must be at least ${EXTRACT_MIN} characters (${len}/${EXTRACT_MIN})`); return false; }
    if (len > EXTRACT_MAX) { setExtractError(`Must be at most ${EXTRACT_MAX} characters (${len}/${EXTRACT_MAX})`); return false; }
    setExtractError(null);
    return true;
  }
  ...
  async function handleClone() {
    ...
    try {
      ...
      if (!res.ok) { setCloneError(data.error ?? "Clone failed. Please try again."); return; }
      onClose();
            ...
            {!isAmazon && (
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  onChange={(e) => { setExtractDescription(e.target.value); setExtractError(null); }}
...
export default function DatasetsClient({ initialDatasets }: Props) {
