...
export function AppNavbar() {
  ...
  // Clear the unread badge as soon as the user visits the notifications page
  useEffect(() => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-5 md:px-8 h-16 bg-background/80 backdrop-blur-md border-b border-border">
      ...
      <div className="hidden md:flex items-center gap-2">
        {[72, 80, 64].map((w) => (
          <div key={w} className="h-8 rounded-md bg-accent/40 animate-pulse" style={{ width: w }} />
        ))}
      </div>

      <div className="hidden md:flex items-center gap-3">
        <div className="h-6 w-24 rounded-full bg-accent/40 animate-pulse" />
        <div className="w-8 h-8 rounded-md bg-accent/40 animate-pulse" />
        <div className="w-7 h-7 rounded-full bg-accent/40 animate-pulse" />
        <div className="h-4 w-16 rounded bg-accent/30 animate-pulse hidden lg:block" />
      </div>

      {/* Mobile skeleton */}
      <div className="md:hidden w-8 h-8 rounded-md bg-accent/40 animate-pulse" />
    </nav>
  );
}

  // Minimal content: signed-out users on /datasets.
  // Gated on `loading` too, so we don't flash full-nav while auth is still resolving.
  const onPublicPage =
  pathname === "/datasets" ||
  pathname.startsWith("/datasets/") ||
  pathname.startsWith("/profile/") ||
  pathname.startsWith("/dataset/web-view/") ||
  pathname.startsWith("/dataset/web-diff/") ||
  pathname.startsWith("/alternate/view-web/");

const showMinimalNav = onPublicPage && !loading && !user;
            ...
            >
              ...
              {loading ? (
                <div className="flex items-center gap-3 px-4 pb-6 border-b border-border">
                  <div className="w-9 h-9 rounded-full bg-accent/40 animate-pulse" />
                  <div className="space-y-1.5">
                    <div className="h-3 w-24 rounded bg-accent/40 animate-pulse" />
                    <div className="h-2.5 w-32 rounded bg-accent/30 animate-pulse" />
                  </div>
                </div>
              ) : (
                <div className="flex items-center gap-3 px-4 pb-6 border-b border-border">
